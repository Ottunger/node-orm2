...
Driver.prototype.hasMany = function (Model, association) {
    const db = this.db.collection(Model.table);
    const driver = this;
    ...
    return {
        has: function (Instance, Associations, conditions, cb) {
            return db.find({
                ...
                for (let i = 0; i < Associations.length; i++) {
                    found = false;
                    for (let j = 0; j < docs[0][association.name].length; j++) {
        ...
        del: function (Instance, Associations, cb) {
            ...
            for (let i = 0; i < Associations.length; i++) {
                let props = {_id: Associations[i][association.model.id]};
...
function convertToDBVal(key, value, timezone) {
    if (value && typeof value.sql_comparator === "function") {
        const val = (key !== "_id" ? value.val : new mongodb.ObjectID(value.val));
        const comp = value.sql_comparator();
        let condition = {};
