...
function extendInstance(Model, Instance, Driver, association, opts, createInstance) {
    ...
    Object.defineProperty(Instance, association.hasAccessor, {
        value: function () {
            let Instances = Array.prototype.slice.apply(arguments);
            ...
            association.model.find(conditions, options, function (err, foundItems) {
                ...
                const mapKeysToString = function (item) {
                    return _.map(association.model.keys, function (k) {
                        return item[k];
                    }).join(',')
                };

                const foundItemsIDs = _(foundItems).map(mapKeysToString).uniq().value();
                const InstancesIDs = _(Instances).map(mapKeysToString).uniq().value();
    ...
    Object.defineProperty(Instance, association.setAccessor, {
        value: function () {
            const items = _.flatten(arguments);
    ...
    Object.defineProperty(Instance, association.addAccessor, {
        value: function () {
            ...
            const run = function () {
                ...
                const saveNextAssociation = function () {
                    if (Associations.length === 0) {
                        return cb(null, savedAssociations);
                    }

                    const Association = Associations.pop();
                    const saveAssociation = function (err) {
                        if (err) {
                            return cb(err);
                        }
